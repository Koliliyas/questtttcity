#!/usr/bin/env python3
"""
Тест создания квеста после исправления дубликатов
"""
import requests
import json
from datetime import datetime

def test_quest_creation_after_fix(base_url="http://questcity.ru/api/v1/"):
    """Тест создания квеста после исправления дубликатов"""
    print(f"🎯 Тест создания квеста после исправления дубликатов")
    print("=" * 60)
    ...
    try:
        ...
        quest_data = {
            "name": "Тестовый квест после исправления",
            "description": "Описание тестового квеста после исправления дубликатов",
...
def main():
    print("🧪 Тест создания квеста после исправления дубликатов")
    print("=" * 60)
    
    success = test_quest_creation_after_fix()
    
    if success:
        print(f"\n{'='*60}")
        print("🎉 Создание квеста работает после исправления дубликатов!")
    else:
        print(f"\n{'='*60}")
        print("❌ Проблемы с созданием квеста остались")
