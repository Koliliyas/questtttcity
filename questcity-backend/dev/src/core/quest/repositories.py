...
from db.models import (Activity, Category, Place, PlaceSettings, Point, Quest,
                       Review, Tool, Vehicle)
...
class QuestRepository(BaseSQLAlchemyRepository[Quest]):
    ...
    async def get_quest_before_update(self, oid: int) -> Quest:
        return await self._session.scalar(
            ...
            .options(selectinload(self._model.merch))
            .options(selectinload(self._model.points).selectinload(Point.places))
        )
