...
class ImageValidateMixin:
    ...
    def validate_image(cls, value: str) -> str:
        if value is not None:
            # Если это URL, пропускаем валидацию
            ...
            if value.startswith("data:"):
                try:
                    # Не изменяем исходное значение; выделяем тело для проверки отдельно
                    _, base64_body = value.split(",", 1)
                    image = Image.open(io.BytesIO(base64.b64decode(base64_body, validate=True)))
                    image.verify()
                ...
                # Пытаемся валидировать как base64
                try:
                    image = Image.open(io.BytesIO(base64.b64decode(value, validate=True)))
                    image.verify()
                except (ValueError, TypeError, IOError):
                    # Если не получилось, считаем что это URL или другой формат
                    pass
