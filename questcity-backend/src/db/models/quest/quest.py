...
class Review(Base):
    ...
    # Ralations:
    user_id: Mapped[int] = mapped_column(ForeignKey("user.id"))
    user: Mapped["User"] = relationship(back_populates="reviews", lazy="noload")  # Изменяем на noload
    quest_id: Mapped[int] = mapped_column(ForeignKey("quest.id"))
    quest: Mapped["Quest"] = relationship(back_populates="reviews", lazy="noload")  # Изменяем на noload
...
class Quest(Base):
    ...
    category: Mapped["Category"] = relationship(back_populates="quests", lazy="noload")  # Изменяем на noload
    group: Mapped[GroupType] = mapped_column(Enum(GroupType))
    vehicle_id: Mapped[int] = mapped_column(ForeignKey("vehicle.id"))
    vehicle: Mapped["Vehicle"] = relationship(back_populates="quests", lazy="noload")  # Изменяем на noload
    ...
    place: Mapped["Place"] = relationship(back_populates="quests", lazy="noload")  # Изменяем на noload

    favorited_by: Mapped[list["Favorite"]] = relationship(back_populates="quest", lazy="noload")  # Изменяем на noload

    # Points:
    points: Mapped[list["Point"]] = relationship(
        back_populates="quest",
        uselist=True,
        lazy="noload",  # Изменяем на noload чтобы избежать циклических ссылок
        cascade="all, delete-orphan",
    )
