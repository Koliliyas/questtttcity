#!/usr/bin/env python3
"""
Исправление всех проблем в серверной базе данных (версия 2)
"""
import psycopg2
import json
...
def fix_all_issues():
    ...
    print("🔧 ИСПРАВЛЕНИЕ ВСЕХ ПРОБЛЕМ В БАЗЕ ДАННЫХ (ВЕРСИЯ 2)")
    print("=" * 80)
    ...
    try:
        ...
        try:
            ...
            # Изменяем тип колонки owner_id на INTEGER с явным приведением
            cursor.execute("ALTER TABLE review ALTER COLUMN owner_id TYPE INTEGER USING owner_id::integer")
        ...
        # 9. Синхронизация данных справочных таблиц (с проверкой структуры)
        print("\n🔧 Синхронизация справочных данных...")
        
        # Синхронизация activity
        try:
            activity_columns = get_table_structure(conn, 'activity')
            print(f"  📋 Структура activity: {activity_columns}")
            
            cursor.execute("DELETE FROM activity")
            if 'description' in activity_columns:
                cursor.execute("""
                ...
                cursor.execute("""
                    ...
                    (1, 'Пешком', 'walking.jpg'),
                    (2, 'На велосипеде', 'bicycle.jpg'),
                    (3, 'На машине', 'car.jpg'),
                    (4, 'На общественном транспорте', 'public_transport.jpg'),
                    (5, 'На самокате', 'scooter.jpg'),
                    (6, 'На роликах', 'rollerblades.jpg'),
                    (7, 'На лыжах', 'skiing.jpg'),
                    (8, 'На коньках', 'skating.jpg'),
                    (9, 'На лодке', 'boat.jpg'),
                    (10, 'На мотоцикле', 'motorcycle.jpg')
                    ON CONFLICT (id) DO NOTHING
                """)
        ...
        try:
            category_columns = get_table_structure(conn, 'category')
            print(f"  📋 Структура category: {category_columns}")
        ...
        # Синхронизация place
        try:
            place_columns = get_table_structure(conn, 'place')
            print(f"  📋 Структура place: {place_columns}")
            ...
            if 'description' in place_columns:
                ...
                cursor.execute("""
                    ...
                    (1, 'Парк', 'park.jpg'),
                    (2, 'Музей', 'museum.jpg'),
                    (3, 'Библиотека', 'library.jpg'),
                    (4, 'Кафе', 'cafe.jpg'),
                    (5, 'Театр', 'theater.jpg'),
                    (6, 'Стадион', 'stadium.jpg'),
                    (7, 'Торговый центр', 'mall.jpg'),
                    (8, 'Университет', 'university.jpg'),
                    (9, 'Больница', 'hospital.jpg'),
                    (10, 'Полиция', 'police.jpg')
                    ON CONFLICT (id) DO NOTHING
                """)
        ...
        try:
            vehicle_columns = get_table_structure(conn, 'vehicle')
            print(f"  📋 Структура vehicle: {vehicle_columns}")
            ...
            if 'description' in vehicle_columns:
                ...
                cursor.execute("""
                    INSERT INTO vehicle (id, name, image) VALUES
                    (1, 'Пешком', 'walking.jpg'),
                    (2, 'Велосипед', 'bicycle.jpg'),
                    (3, 'Автомобиль', 'car.jpg'),
                    (4, 'Общественный транспорт', 'public_transport.jpg'),
                    (5, 'Самокат', 'scooter.jpg'),
                    (6, 'Ролики', 'rollerblades.jpg')
                    ON CONFLICT (id) DO NOTHING
                """)
        ...
        # Синхронизация tool
        try:
            tool_columns = get_table_structure(conn, 'tool')
            print(f"  📋 Структура tool: {tool_columns}")
            
            cursor.execute("DELETE FROM tool")
            if 'description' in tool_columns:
                cursor.execute("""
