  ...
  List<List<int>> _getSelectedIndexes(QuestDetailModel quest) {
    ...
    if (quest.mainPreferences != null) {
      return [
        ...
        ], // categoryId начинается с 1, а индексы с 0
        [
          quest.mainPreferences!.vehicleId - 1
        ], // vehicleId начинается с 1, а индексы с 0
        [
          quest.mainPreferences!.placeId - 1
        ], // placeId начинается с 1, а индексы с 0
        [
          quest.mainPreferences!.group - 1
        ], // group начинается с 1, а индексы с 0
      ];
    }
    return [
      [0],
  ...
  MainPreferencesUpdate _buildMainPreferencesUpdate(
    ...
    final categoryId =
        selectedIndexes.isNotEmpty && selectedIndexes[0].isNotEmpty
            ? selectedIndexes[0][0] + 1
            : 1;
    final vehicleId =
        selectedIndexes.length > 1 && selectedIndexes[1].isNotEmpty
            ? selectedIndexes[1][0] + 1
            : 1;
    final placeId = selectedIndexes.length > 2 && selectedIndexes[2].isNotEmpty
        ? selectedIndexes[2][0] + 1
        : 1;
    final group = selectedIndexes.length > 3 && selectedIndexes[3].isNotEmpty
    ...
    return MainPreferencesUpdate(
      ...
      vehicleId: vehicleId,
      placeId: placeId,
      group: _getGroupString(group),
      timeframe: _getTimeframeString(currentState.difficultyLevel),
      level: currentState.difficultyLevel,
      mileage: _getMileageString(currentState.difficultyLevel),
