...
from src.api.modules.chat import responses
from src.api.modules.chat.schemas import (ChatPreviewResponseSchema,
                                      ChatResponseSchema,
...
from src.core.authentication.dependencies import (
    get_user_with_role,
    get_websocket_user_with_chat_access,
)
from src.core.chat import exceptions as chat_exc
...
async def chat_websocket_endpoint(
    ...
    """
    WebSocket endpoint для чата с JWT аутентификацией.
    
    Использование:
    - Query параметр: ws://localhost:8000/api/v1/chats/{chat_id}?token=your_jwt_token
    - Header: Authorization: Bearer your_jwt_token
    """
    # Аутентифицируем пользователя и проверяем доступ к чату
    user = await get_websocket_user_with_chat_access(websocket, chat_id)
    
    # Подключаем аутентифицированного пользователя
    ...
    try:
        ...
        await ws_service.disconnect(chat_id, user.id)
